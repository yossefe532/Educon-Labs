...
export function analyzeConflicts(bookings, candidate, halls, excludeId = null) {
  ...
  if (candidate.type === 'single') {
    ...
    if (conflicts.length) {
      ...
      const confEnd = conf.type === 'recurring' && conf.dayTimes ? conf.dayTimes[weekdayOf(candidate.date)]?.end : conf.end
      const confStart = conf.type === 'recurring' && conf.dayTimes ? conf.dayTimes[weekdayOf(candidate.date)]?.start : conf.start
      results.conflicting.push({ date: candidate.date, conflictWith: conf, confStart, confEnd })
    ...
    for (const slot of (candidate.slots || [])) {
      ...
      if (conflicts.length) {
        results.conflicting.push({ date: slot.date, conflictWith: conflicts[0] })
        for (const h of halls) {
          if (h.id === candidate.hallId) continue
    ...
    while (d <= ed) {
      ...
      for (const di of days) {
        if (weekdayOf(ds) === di) {
          ...
          const conflicts = findConflicts(bookings, test, excludeId)
          if (conflicts.length) {
            results.conflicting.push({ date: ds, conflictWith: conflicts[0], dayOfWeek: di })
            for (const h of halls) {
              if (h.id === candidate.hallId) continue
              const t2 = { ...test, hallId: h.id }
              if (!findConflicts(bookings, t2, excludeId).length) {
